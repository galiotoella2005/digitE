...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    tmemplate has a document node to match on-->
        <html>
            <head>
                ...
                -->
                <link rel="stylesheet" type="text/css" href="behrendletters.css"/>
            </head>
            <body>
    ...
    <xsl:template match="letter" mode="toc">
        <tr>
            ...
            <td><a href="#{substring-before(//headLine, ' ')}"><xsl:value-of select="string-join(distinct-values(descendant::person/string()))"/></a></td><!--second column data cell: to hold a sorted, string-joined list of persons mentioned. -->
            <td><a href="#{substring-before(//headLine, ' ')}"><xsl:value-of select="string-join(distinct-values(descendant::location/string()))"/></a></td><!--third column data cell: to hold a sorted, string-joined list of locations mentioned.-->
    ...
    <xsl:template match="letter">
        <section class="readingView" id="{substring-before(//headLine, ' ')}">
            <h2><xsl:value-of select="descendant::headLine"/></h2>
